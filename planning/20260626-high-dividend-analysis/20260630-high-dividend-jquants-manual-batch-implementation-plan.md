# 高配当分析ツール本格化 実装計画: J-Quants + EDINET手動batch + BFFスコア計算

## 1. 結論

本人の個人利用/PoCに限定し、J-Quants API無料範囲とEDINET APIを組み合わせ、最終成果物をCSVに統一する手動batch方式で、任意タイミングに実在データを取得・補完できる高配当分析ツールへ進める。

外部データ源の役割は分ける。J-Quantsは株価・銘柄基本情報・取得できる財務サマリーを担当し、EDINETは有価証券報告書XBRLから営業CF・投資CFなどの財務/CF補完を担当する。どちらのAPIで取得した値も、BFFへ渡す段階ではCSVに集約する。これにより、外部APIそのものは複数でも、BFF以降の取り込み口はCSVに統一でき、人間が手間なく更新・確認・補完できる。
...
| 外部データ | J-Quants API無料範囲 + EDINET APIの併用を採用する |
| J-Quantsの役割 | 株価、銘柄基本情報、取得可能な財務サマリー、直近利回り計算に必要な値の取得候補 |
| EDINETの役割 | 有価証券報告書XBRLから営業CF・投資CF・配当/財務関連値を無料補完できるか検証する |
| API統合方針 | 外部APIを単一化するのではなく、BFFへの入力成果物をCSVに統一する |
| 補完データ | J-Quants/EDINETで不足する項目は手動CSVで補完する。実データCSVはGit管理せず、BFF repo内のgitignore/private localに置く |
...
Manual Batch
    ↓
J-Quants API Free Scope + EDINET API + Candidate CSV/Manual CSV
    ↓
Generated Unified CSV (batch final artifact)
    ↓
...
- J-Quants APIキーとEDINET APIキーはBFF/サーバー側、またはローカルbatch実行環境だけに置く。
- 手動batchはBFFリポジトリ側のCLIまたはnpm scriptとして実装する。
- batchは複数データ源を直接BFF APIへ流さず、必ず統一CSVを生成してからimportする。
...
```bash
pnpm dividend-analysis:fetch-jquants --symbols 8058,9432,8306 --as-of 2026-06-30 --out data/dividend-analysis/private/jquants-raw-20260630.json
pnpm dividend-analysis:fetch-edinet --symbols 8058,9432,8306 --years 10 --out data/dividend-analysis/private/edinet-raw-20260630/
pnpm dividend-analysis:generate-csv --symbols 8058,9432,8306 --jquants data/dividend-analysis/private/jquants-raw-20260630.json --edinet data/dividend-analysis/private/edinet-raw-20260630/ --out data/dividend-analysis/private/dividend-analysis-unified.csv
pnpm dividend-analysis:import-csv --file data/dividend-analysis/private/dividend-analysis-unified.csv
...
実装時にコマンド名はBFFの既存規約へ合わせる。batchの最終成果物はCSVにする。`generate-csv` はローカルで実行し、J-Quants/EDINET等で埋められる列を自動入力し、APIで取れない列は空欄ではなく `TODO:<固定文言>` または `notAvailableReason` 付きで出力する。人間はそのCSVを確認・必要箇所だけ補完して `import-csv` に渡す。更新時は「fetch-jquants → fetch-edinet → generate-csv → 人間確認/補完 → import-csv → recalculate」の一連の手順でよい。
...
- EDINETから取得したXBRL zip/raw抽出結果、営業CF・投資CFなど
- 手動CSVから補完した配当履歴、減配履歴、FCF関連値など
- `sourceType`: `jquants_api` / `edinet_api` / `manual_csv`
- `sourceName`
- `dataAsOfDate`
- `fetchedAt`
- `symbolId`
...
原則は「J-Quants/EDINETで取れないものは全部CSV補完」でよい。ただし何が取れないかは実APIスパイクで確定する。したがって、BFF Issueでは最初のタスクを「BFF現状mock項目のJ-Quants無料範囲 + EDINET補填マトリックス作成」とする。
...
2. 高配当上位候補リスト
   - Yahoo!ファイナンスの高配当利回りスクリーニング（例: `https://finance.yahoo.co.jp/stocks/screening/highdividend?sort=-dividendYield`）などから上位50社候補を自動取得する仕組みを作る。
   - これは分析データ本体ではなく、対象銘柄候補の取得元として扱う。
   - ただし利用条件に反しない範囲で、まずローカル手動コマンドとして実装し、失敗時は人間が取得した候補CSVのimportでフォールバックする。
...
- J-Quants/EDINETで取れない項目は全部CSV補完対象にする。補完対象はスパイク結果で確定し、CSV schemaに反映する。
- FCFはまずEDINET APIで無料自動取得を試し、失敗したものだけTODO固定文言つきで手動補完に回す。

### 7.1 API併用時の更新フロー

採用方針は「外部APIはJ-Quants + EDINETの併用、BFF入力はCSV統一」。人間が手間なく更新できるよう、通常更新は以下の1フローに寄せる。

```bash
pnpm dividend-analysis:update-csv --symbols-file data/dividend-analysis/private/symbols.csv --as-of 2026-06-30 --out data/dividend-analysis/private/dividend-analysis-unified.csv
```

`update-csv` は内部で以下を行う想定。

1. J-Quantsから銘柄基本情報、株価、取得可能な財務サマリーを取得する。
2. EDINETから対象銘柄の有価証券報告書XBRLを取得し、営業CF・投資CFなどを抽出する。
3. 高配当候補リストを自動取得し、固定銘柄CSVまたは候補CSVと突合する。
4. 統一CSVを生成する。
5. 自動取得できなかった列に `TODO:<固定文言>` と `notAvailableReason` を入れる。

人間の作業は、生成されたCSVのTODO列だけを確認・補完し、同じCSVを `import-csv` に渡すことに限定する。
...
この設計により、APIが複数でも運用上の更新入口は `update-csv` とCSV確認に集約できる。
...
/Users/ynaragin/git/bff
  data/
    dividend-analysis/
      templates/
        dividend-analysis-unified.example.csv # Git管理OK
      private/
        dividend-analysis-unified.csv   # gitignore
        jquants-raw-YYYYMMDD.json       # gitignore
        edinet-raw-YYYYMMDD/            # gitignore
```
...
- [ ] 手動batchでEDINET APIから対象銘柄のXBRL/rawを取得できる。
- [ ] `update-csv` 相当の1コマンドで、J-Quants + EDINET + 候補/固定銘柄入力から統一CSVを生成できる。
- [ ] EDINET APIから営業CF・投資CFを抽出し、FCFを無料自動補完できるかSpikeする。
- [ ] BFF現状mock項目について、J-Quants無料範囲・EDINET APIで補填できる項目とCSV補完が必要な項目のマトリックスを作成する。
...
- [ ] Yahoo!ファイナンス等の高配当候補取得は自動化してよい。まずローカル手動コマンドとして実装し、失敗時は候補CSV importでフォールバックできる。
...
| 高配当候補リスト取得 | Yahoo!ファイナンス等から上位50社を取る場合、サイト仕様変更や取得失敗が起き得る | 分析データ本体ではなく候補銘柄リストとして扱う。ローカル手動コマンドで自動取得し、失敗時は候補CSV importへフォールバックする |
...
- 目的: J-Quants無料範囲 + EDINET API + 手動CSVを使った手動batchと、BFF内スコア計算を実装するためのIssue案を作る。
- 重要前提:
  - 本人の個人利用/PoCのみ。
  - 外部APIは実データ取得だけに使う。
  - 外部APIはJ-Quants + EDINETを併用し、BFFへの入力は統一CSVに寄せる。
  - スコア計算はBFFが担当する。
  - 画面リクエスト中にJ-Quants APIへ同期アクセスしない。
  - APIキーはFEへ露出しない。
...
- 期待成果物:
  ...
  - J-Quants無料範囲 + EDINET APIでBFF現状mockをどこまで補填できるかのスパイク設計。
  - EDINET APIで営業CF・投資CFを抽出してFCFを無料自動補完するスパイク設計。
  - APIで取れない項目を全部CSV補完対象にする設計。
  - ローカルコマンドで統一CSVを生成する設計。
...
1. J-Quants API + EDINET API併用での実現可能性を確認する。
   - BFFの現状mock項目を、J-Quants無料範囲とEDINET APIでどこまで補填できるかを見る。
...
3. 手動CSV補完は、基本的にJ-Quants/EDINETで取れないもの全部を対象にする。
...
5. 実データCSVはローカルコマンドで生成するイメージにする。
   ...
   - 通常更新は `update-csv` 相当の1コマンドに寄せ、CSV確認・補完の手間を最小化する。
6. BFFスコア計算式の閾値は既存方針から変更しない。
7. 金融業のFCF N/A換算ルールは既存方針から変更しない。
8. 対象銘柄は、固定20銘柄と高配当上位50候補を同時並行で扱えるようにする。
   - 高配当上位50候補は、Yahoo!ファイナンス高配当利回りスクリーニング等から自動取得する仕組みを作ってよい。
   - まずローカル手動コマンドとして実装し、取得失敗時やサイト仕様変更時は候補CSV importへフォールバックする。

### 残確認

- J-Quants無料範囲で実際に取れる項目。
- EDINET APIで固定20銘柄の営業CF・投資CFをどれだけ安定抽出できるか。
- J-Quants + EDINET併用時の統一CSV schemaと `update-csv` コマンド名。
