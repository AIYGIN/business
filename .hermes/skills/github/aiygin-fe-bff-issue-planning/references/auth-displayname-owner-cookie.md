...
- このメモは、ユーザーが明示的に「Google displayName を TODO 所有者として使う」と指定した場合の例外設計である。
- ユーザーが「ログイン済みユーザーIDをCookieから取得してTODOに紐づける」と指定した場合は、displayNameではなくCookie JWTから復元した安定ID（例: `CurrentUser.subject`）を `todos.owner_user_id` に保存・検索する方針を優先する。
