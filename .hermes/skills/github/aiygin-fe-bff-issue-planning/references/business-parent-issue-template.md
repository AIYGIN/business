...
- DB実装を含むIssueでは `## DBテーブル案` を設け、table名、主要列、owner key、index、migration/DDL方針を記載する
- TODO等のユーザー所有データでは、FEからuserIdを受け取らず、BFFがHttpOnly Cookie内JWTを検証して復元したログイン済みユーザーIDをowner key（例: `owner_user_id`）としてDBへ保存・検索する
