...
- FE/BFF 調査では、コード構成だけでなく、子 Issue 作成用の repo-local asset（skill / template / script / workflow）も確認し、path、使い方、見つからない場合の fallback 方針を調査結果に含める。詳細メモは `references/aiygin-child-issue-delegation-assets.md` を参照する。
