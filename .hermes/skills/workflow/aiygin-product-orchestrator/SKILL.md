...
Hermes から Codex を起動する場合は、Codex CLI の性質上 `pty=true` で実行する。
...
Hermes gateway / service コンテキストで Codex sandbox が壊れる場合だけ、`--sandbox danger-full-access` を検討する。その場合も必ず作業ディレクトリを限定し、事前の git status、事後の git diff / test / PR URL 検証を行う。
