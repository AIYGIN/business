...
Hermes から Codex を起動する場合は、Codex CLI の性質上 `pty=true` で実行し、`codex ...` で直接起動する。

```text
terminal(command="codex exec --full-auto '<prompt>'", workdir=BFF_REPO, pty=true, timeout=600)
terminal(command="codex exec --full-auto '<prompt>'", workdir=FE_REPO, pty=true, timeout=600)
```
...
```bash
agent-memory write --content "aiygin codex: <phase> 開始。repo=<repo> command=codex exec --full-auto / input=<要約>"
agent-memory write --content "aiygin codex: <phase> 完了。repo=<repo> result=<要約> / urls=<Issue or PR URL> / next=<次作業>"
agent-memory write --content "aiygin codex: <phase> 失敗。repo=<repo> exit=<code> / reason=<理由> / next=<再開条件>"
```

未完了TODOや人間確認待ちは `agent-memory scratchpad add` に残す。

Hermes gateway / service コンテキストで Codex sandbox が壊れる場合だけ、`--sandbox danger-full-access` を検討する。その場合も `codex exec --sandbox danger-full-access "<task>"` の形を維持し、作業ディレクトリを限定し、事前の git status、事後の git diff / test / PR URL 検証を行う。
...
- [ ] Hermes から Codex を起動した場合、`codex ...` で実行している。
